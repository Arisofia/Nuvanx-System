...
async function main() {
  ...
  for (const adAccountId of adAccountIds) {
    try {
      ...
      console.error(`[meta-backfill] Error processing account ${adAccountId}:`, err.message);
      if (!firstError) firstError = err;
    }
  }

  await db.end();
  console.log(`[meta-backfill] Completed. Total daily insights upserted: ${totalUpserted}`);
