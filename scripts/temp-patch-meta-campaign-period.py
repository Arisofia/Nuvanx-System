...
infer_start = source.index('function inferStatusFromLastLead')
infer_end = source.index('export function buildCampaignsTimeRange', infer_start)
source = source[:infer_start] + source[infer_end:]

db_start = source.index('async function fetchDbCampaigns')
fallback_start = source.index('async function fetchMetaCampaignsFallback', db_start)
source = source[:db_start] + source[fallback_start:]
...
forbidden = [
    ...
    'async function fetchDbCampaigns',
    'inferStatusFromLastLead',
]
remaining = [value for value in forbidden if value in source]
if remaining:
    raise SystemExit(f'Forbidden legacy campaign mixing remains: {remaining}')
...
obsolete_start_marker = "  it('scopes campaign CRM fallback through clinic member user IDs instead of a non-existent view clinic_id', () => {"
obsolete_end_marker = "\n  it('keeps persisted Organic and Instagram reads clinic-required', () => {"
obsolete_start = tenant.index(obsolete_start_marker)
obsolete_end = tenant.index(obsolete_end_marker, obsolete_start)
tenant = tenant[:obsolete_start] + tenant[obsolete_end:]
...
describe('Meta operational campaign period contract', () => {
  it('keeps CRM and Doctoralia history out of the operational Meta campaign endpoint', () => {
    ...
    expect(api).not.toContain('async function fetchDbCampaigns');
    expect(api).not.toContain('inferStatusFromLastLead');
