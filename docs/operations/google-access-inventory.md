...
| Google Business Profile review-request WordPress code | Local/Nuvanx-System workspace MU plugins | `nuvanx-google-review-request.php` / `-v2.php` | **CODE_ONLY** | Review links now default to the API-confirmed Place IDs for Chamberí and Goya. This is governed site behavior and uses verified non-secret identifiers, but the PHP itself is not proof of GBP provider permission. |
