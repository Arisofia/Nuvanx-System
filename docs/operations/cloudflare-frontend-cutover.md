...
2. In GitHub Actions run **Cloudflare Frontend Acceptance** on the exact approved `main` commit.
...
5. Keep this temporary acceptance hostname out of search indexing. Do not change production DNS yet.
...
1. Attach the final frontend custom domain in Cloudflare.
2. Record the DNS change and previous value for rollback.
3. Verify TLS, SPA deep links, security headers, Auth redirects, API/CORS and authenticated flows on the custom domain.
4. Confirm the deployed Cloudflare version corresponds to the approved Git commit.
5. Keep Vercel intact through the acceptance window.
