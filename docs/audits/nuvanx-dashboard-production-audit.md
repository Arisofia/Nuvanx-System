...
| `frontend/src/components/dashboard/RealROISection.tsx` | 56-76 | CAC/revenue per lead null render as `0`; confidence assumes percentage. | CAC/revenue can mislead budget decisions. | Render blocked/unavailable until attribution is verified. |
...
| `supabase/functions/api/index.ts` | 2529-2633 | Meta trends uses live, DB, or cache fallback and can return `success: true` degraded payload. | UI does not consistently surface degraded/cache status. | Require card-level degraded warnings and freshness. |
