-- Pre-bridge for the canonical public.leads.reply_delay_minutes INTEGER contract.
--
-- Production already uses INTEGER and this migration must therefore be a schema
-- no-op there. Historical clean replay reaches this point with NUMERIC while
-- public views already expose reply_delay_minutes as INTEGER. Reconcile that
-- exact known drift before 20260901160000 rebuilds reporting views.
--
-- The later applied migration 20260902091000 remains immutable and becomes a
-- no-op after this bridge during clean replay.
