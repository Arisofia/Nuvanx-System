-- Restored from the authoritative production migration ledger for version 20260820163616.
-- This supersedes the previous funnel function so cancelled/no-show valuations are
-- not promoted as attended. Production already has this version; fresh previews
-- must execute the same canonical schema/function change.
...
BEGIN
  ...
  SET funnel_stage = CASE
        ...
        WHEN lower(coalesce(a.subject,'') || ' ' || coalesce(a.treatment,'') || ' ' || coalesce(a.appointment_type,'')) ~ '(valoraci|primera visita|primera)'
             AND a.is_cancelled IS TRUE THEN NULL
        WHEN lower(coalesce(a.subject,'') || ' ' || coalesce(a.treatment,'') || ' ' || coalesce(a.appointment_type,'')) ~ '(valoraci|primera visita|primera)'
             AND a.is_cancelled IS FALSE
             AND lower(trim(a.estado)) <> 'no acude' THEN 'asistio'
        ELSE NULL
      END,
      funnel_stage_reason = CASE
        ...
        WHEN lower(coalesce(a.subject,'') || ' ' || coalesce(a.treatment,'') || ' ' || coalesce(a.appointment_type,'')) ~ '(valoraci|primera visita|primera)'
             AND a.is_cancelled IS FALSE
             AND lower(trim(a.estado)) = 'no acude' THEN 'valoracion_no_asistio'
        WHEN lower(coalesce(a.subject,'') || ' ' || coalesce(a.treatment,'') || ' ' || coalesce(a.appointment_type,'')) ~ '(valoraci|primera visita|primera)'
