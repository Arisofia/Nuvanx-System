...
DO $$
BEGIN
  -- In preview / core-only databases, public.leads and vw_lead_traceability may
  -- not exist yet. In that case, skip creating get_campaign_roi to avoid
  -- migration failures while still allowing full production migrations.
  IF to_regclass('public.vw_lead_traceability') IS NULL
     OR to_regclass('public.leads') IS NULL THEN
    RAISE NOTICE 'Skipping get_campaign_roi creation: vw_lead_traceability or public.leads does not exist';
    RETURN;
  END IF;
