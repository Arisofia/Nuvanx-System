...
BEGIN
  IF to_regclass('public.financial_settlements') IS NULL THEN
    RAISE NOTICE 'Skipping financial_settlements revenue attribution: table does not exist yet';
    RETURN;
  END IF;

  ALTER TABLE IF EXISTS public.financial_settlements ADD COLUMN IF NOT EXISTS lead_id UUID REFERENCES leads(id);
