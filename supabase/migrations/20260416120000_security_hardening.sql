-- =============================================================================
-- Migration: Security hardening — RLS policies, stale data reset, FK cleanup
-- =============================================================================
-- Addresses:
--   1. credentials: no policies → add explicit deny for anon/authenticated,
--      service_role bypass for backend
--   2. leads: no policies → same treatment (PII / GDPR)
--   3. integrations: SELECT-only policy → complete with service_role bypass
--   4. playbook_executions: drop duplicate FK (pe_playbook_fk) if exists
--   5. dashboard_metrics: reset stale seed row to zeros
--   6. kpi_definitions / kpi_values: add service_role policies so backend can
--      write when ready
-- =============================================================================
...
-- Authenticated: owner-only read (belt-and-suspenders — the browser should
-- never call this directly, but if it does it can only see its own rows).
...
-- Anon: explicit deny for all operations
...
-- Authenticated: owner-only SELECT (so browser can display own leads if needed)
...
-- The migration defines REFERENCES (inline FK → auto-named
-- playbook_executions_playbook_id_fkey). A second constraint pe_playbook_fk
-- was added manually and points to the same column — drop it.
ALTER TABLE public.playbook_executions
  DROP CONSTRAINT IF EXISTS pe_playbook_fk;


-- ─── 5. dashboard_metrics: reset stale seed row ───────────────────────────
-- connected_integrations=7 when credentials=0 breaks coherence.
-- Reset to zeros; the backend /api/dashboard/metrics computes live values.
