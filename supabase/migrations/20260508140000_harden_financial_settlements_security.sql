...
BEGIN
  IF to_regclass('public.financial_settlements') IS NULL THEN
    RAISE NOTICE 'Skipping financial_settlements security hardening: table does not exist yet';
    RETURN;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'financial_settlements'
      AND policyname = 'financial_settlements_select_clinic'
  ) THEN
