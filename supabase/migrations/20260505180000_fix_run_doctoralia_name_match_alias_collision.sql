  ...
  LOOP
    ...
    FOR lead_row IN
      SELECT
        ...
        lower(
          extensions.unaccent(
            regexp_replace(COALESCE(ld.name, ''), '\s+', ' ', 'g')
          )
        ) AS normalized_name,
    ...
    LOOP
      ...
      first_token_match := lead_token_count >= 1
                           AND lead_tokens[1] = ANY (patient_row.name_tokens);

      last_token_match := lead_token_count >= 1
                          AND lead_tokens[lead_token_count] = ANY (patient_row.name_tokens);
      ...
      IF best_lid IS NULL
         ...
         OR (rank_score = best_score AND ph_match) THEN
