-- Align Doctoralia appointment state with the canonical public.appointment_status enum.
-- Production failure observed when TEXT CASE output was assigned to the enum column.
  ...
  updated AS (
    ...
    SET
      ...
      appointment_status = CASE
        WHEN lower(pm.estado) IN ('realizada','pagada') THEN 'showed'::public.appointment_status
        WHEN lower(pm.estado) = 'no acude' THEN 'no_show'::public.appointment_status
