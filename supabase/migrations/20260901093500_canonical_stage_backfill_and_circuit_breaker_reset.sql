...
-- 1. Backfill stage_canonical for leads where stage_canonical IS NULL or incorrectly set to 'lead'
-- This recompute canonical stages for rows that were incorrectly assigned 'lead' by
-- earlier unconditional backfill (20260901080200), ensuring evidence-based mapping
-- (won, appointment, WhatsApp, contact) runs for the 792 affected leads.
WITH resolved AS (
  SELECT
    l.id AS lead_id,
    CASE
      ...
      WHEN l.stage IS NOT NULL AND lower(trim(l.stage)) NOT IN ('', 'lead')
        THEN NULL
...
-- 2. Ensure newly inserted or updated leads derive canonical stage from available evidence
-- Trigger runs on both INSERT and UPDATE to keep stage_canonical synchronized when
-- lifecycle handlers (e.g., WhatsApp paths in api/index.ts) update stage and evidence fields.
...
AS $$
BEGIN
  IF NEW.stage_canonical IS NULL OR (TG_OP = 'UPDATE' AND OLD.stage_canonical IS DISTINCT FROM NEW.stage_canonical) THEN
...
-- Update both 'open' and 'half_open' states to handle databases that already ran the earlier
-- migration (20260901080000) which unconditionally set it to half_open with failure_count = 0.
...
WHERE provider = 'google'
  AND breaker_state IN ('open', 'half_open');

-- 5. Preserve authenticated and service_role access for nvx_get_hubspot_marketing_contact_monitor
-- The dashboard monitor (HubSpotMarketingContactMonitor.tsx) calls this RPC directly
-- through the session-scoped Supabase client, so authenticated access must be preserved.
-- Revoking authenticated would cause permission errors for signed-in users.
