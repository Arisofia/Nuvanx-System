...
journey_raw as (
  select
    ...
    t.start_time,
    case
      when t.start_time is null then null::timestamptz
      else (
        a.appointment_date::text || ' ' ||
        case when length(t.start_time)=4 then '0' || t.start_time else t.start_time end
...
journey_candidates as (
  select
    r.lead_id,
    r.appointment_ingestion_id,
    coalesce(
      nullif(btrim(r.appointment_id),''),
      nullif(btrim(r.source_key),''),
      md5(concat_ws('|',
        coalesce(r.doctoralia_id,r.phone_normalized,''),
        r.appointment_date::text,
        coalesce(btrim(r.appointment_time),''),
        coalesce(btrim(r.appointment_type),''),
        coalesce(btrim(r.subject),''),
        coalesce(btrim(r.agenda),''),
        coalesce(btrim(r.clinic),''),
        coalesce(btrim(r.treatment),'')
      ))
    ) as appointment_key,
    r.appointment_date,
    r.appointment_time,
  ...
  where coalesce(r.is_cancelled,false)=false
    and lower(btrim(coalesce(nullif(btrim(r.status),''),nullif(btrim(r.estado),''),''))) not in (
      'anulada','anulado','cancelada','cancelado','cancelled','canceled',
      'no acude','no acudió','no acudio','no_show','no show','noshow'
    )
    and (
      r.appointment_date > r.lead_created_at::date
      or (
        r.appointment_date = r.lead_created_at::date
        and r.appointment_at is not null
        and r.appointment_at >= r.lead_created_at
      )
    )
),
journey_dedup as (
...
journey_ranked as (
  select
    ...
    row_number() over (
      ...
      order by
        d.appointment_date,
        d.appointment_at nulls last,
        d.appointment_ingestion_id
    ) as visit_number
