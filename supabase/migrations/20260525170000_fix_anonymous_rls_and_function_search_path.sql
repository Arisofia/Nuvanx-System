...
BEGIN
  ...
  IF target_table IS NOT NULL AND pg_temp.has_column(target_table, 'user_id') THEN
    PERFORM pg_temp.recreate_select_policy(
      ...
      '(SELECT CASE lower(auth.jwt() ->> ''is_anonymous'') WHEN ''false'' THEN false WHEN ''f'' THEN false WHEN ''0'' THEN false WHEN ''no'' THEN false WHEN ''off'' THEN false WHEN ''true'' THEN true WHEN ''t'' THEN true WHEN ''1'' THEN true WHEN ''yes'' THEN true WHEN ''on'' THEN true ELSE true END) IS NOT TRUE AND (SELECT auth.uid()) = user_id'
    );
  ELSE
...
-- Clinic-scoped SELECT policies. public.produccion_intermediarios is intentionally
-- excluded here and handled in the follow-up initplan-specific migration.
DO $$
DECLARE
  ...
  LOOP
    ...
    IF target_table IS NOT NULL AND pg_temp.has_column(target_table, 'clinic_id') THEN
      PERFORM pg_temp.recreate_select_policy(
        ...
        '(SELECT CASE lower(auth.jwt() ->> ''is_anonymous'') WHEN ''false'' THEN false WHEN ''f'' THEN false WHEN ''0'' THEN false WHEN ''no'' THEN false WHEN ''off'' THEN false WHEN ''true'' THEN true WHEN ''t'' THEN true WHEN ''1'' THEN true WHEN ''yes'' THEN true WHEN ''on'' THEN true ELSE true END) IS NOT TRUE AND clinic_id = (SELECT public.current_clinic_id())'
      );
    ELSE
...
BEGIN
  ...
  IF target_table IS NOT NULL AND pg_temp.has_column(target_table, 'id') THEN
    PERFORM pg_temp.recreate_select_policy(
      target_table,
      'clinics_select',
      '(SELECT CASE lower(auth.jwt() ->> ''is_anonymous'') WHEN ''false'' THEN false WHEN ''f'' THEN false WHEN ''0'' THEN false WHEN ''no'' THEN false WHEN ''off'' THEN false WHEN ''true'' THEN true WHEN ''t'' THEN true WHEN ''1'' THEN true WHEN ''yes'' THEN true WHEN ''on'' THEN true ELSE true END) IS NOT TRUE AND id = (SELECT public.current_clinic_id())'
    );
  ELSE
