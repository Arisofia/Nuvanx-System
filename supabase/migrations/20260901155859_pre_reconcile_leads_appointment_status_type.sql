-- Forward-only pre-reconciliation for public.leads.appointment_status.
--
-- This version is intentionally ordered before the already-applied immutable
-- 20260901155900 bridge. Production already carries the canonical enum, so this
-- migration is a no-op there. On a clean replay it performs the strengthened
-- conversion first, including the exact dependent-view allowlist and the one
-- required enum-to-text compatibility rewrite. The historical 155900 migration
-- then observes the canonical enum and becomes a no-op without being modified.
...
BEGIN
  ...
  -- Strictly allow only the dependency graph observed in clean replay.
  SELECT array_agg(
