-- Clean-replay bridge immediately before immutable 20260901160000.
--
-- Historical replay already has vw_lead_traceability with a 43-column signature
-- that is incompatible with CREATE OR REPLACE type changes performed by 160000.
-- Production already has the canonical signature, so this migration is a no-op
-- there. Only the exact observed historical Preview signature may be dropped.

BEGIN;

DO $pre_drop_legacy_lead_traceability$
DECLARE
  v_signature text;
  v_reloptions text[];
  v_owner_name text;
  v_view_comment text;
  v_acl text[];
  v_historical_acl constant text[] := ARRAY[
    'anon:DELETE:plain',
    'anon:INSERT:plain',
...
BEGIN
  ...
  -- Canonical Production: ledger-only/no schema mutation.
  IF v_signature = v_canonical_signature THEN
    RETURN;
  END IF;

  IF v_signature IS DISTINCT FROM v_historical_signature THEN
    RAISE EXCEPTION 'Unexpected vw_lead_traceability signature before 160000:%', E'\n' || coalesce(v_signature, '<missing>');
  END IF;

  IF v_owner_name IS DISTINCT FROM 'postgres' THEN
    RAISE EXCEPTION 'Unexpected vw_lead_traceability owner before 160000: %', v_owner_name;
  END IF;

  IF NOT ('security_invoker=true' = ANY(COALESCE(v_reloptions, ARRAY[]::text[]))) THEN
    RAISE EXCEPTION 'Historical vw_lead_traceability must be security_invoker=true before 160000';
  END IF;

  IF v_view_comment IS NOT NULL THEN
    RAISE EXCEPTION 'Unexpected historical vw_lead_traceability comment before 160000: %', v_view_comment;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Cannot pre-drop historical vw_lead_traceability: column-level ACLs detected';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Cannot pre-drop historical vw_lead_traceability: dependent view exists';
  END IF;
  ...
  IF v_acl IS DISTINCT FROM v_historical_acl THEN
    RAISE EXCEPTION 'Unexpected historical vw_lead_traceability ACL before 160000: %', v_acl;
  END IF;

  DROP VIEW public.vw_lead_traceability;
END;
$pre_drop_legacy_lead_traceability$;
