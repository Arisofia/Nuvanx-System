...
async function generatedServiceAccount(): Promise<string> {
...
describe("shared Google Ads runtime auth", () => {
  ...
  it("does not expose OAuth provider payloads in failures", async () => {
    ...
    await expect(resolveGoogleAdsAuth({
      ...
      oauthClientSecret: "client-secret",
      oauthRefreshToken: "refresh-token",
    }, fetchImpl)).rejects.toSatisfy((error: unknown) => {
      expect(error).toBeInstanceOf(GoogleAdsAuthFailure);
      expect(String((error as Error).message)).not.toContain("sensitive-provider-detail");
      expect(String((error as Error).message)).not.toContain("invalid_grant");
      return true;
    });
  });
});
