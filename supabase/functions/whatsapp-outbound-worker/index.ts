...
function loadKeyring(): Record<string, string> {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("queue_keyring_invalid");
...
async function decryptMessage(row: ClaimRow, keyring: Record<string, string>): Promise<string> {
  ...
  const key = await crypto.subtle.importKey("raw", ownedArrayBuffer(keyBytes), { name: "AES-GCM" }, false, ["decrypt"]);
...
Deno.serve(async (req: Request) => {
  ...
  if (!SUPABASE_URL || !SERVICE_ROLE) return reply(500, { success: false, message: "Worker runtime configuration unavailable" });
  if (bearerToken(req) !== SERVICE_ROLE) return reply(401, { success: false, message: "Service-role authorization required" });
  ...
  if (!accessToken || !phoneNumberId) return reply(503, { success: false, message: "WhatsApp provider configuration unavailable" });
  ...
  const { data: claimed, error: claimError } = await admin.rpc("nvx_claim_whatsapp_outbound_payload", { p_limit: limit });
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (await sha256Hex(message) !== row.message_sha256) throw new Error("message_fingerprint_mismatch");
    } catch {
      // No provider attempt has started. Keep the claim encrypted so a temporary
      // keyring/configuration problem can be retried safely by bounded claim recovery.
      deferred += 1;
      continue;
    }
