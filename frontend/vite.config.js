...
export default ({ mode }) => {
  ...
  return defineConfig({
    ...
    server: {
      ...
      proxy: {
        '/api': {
          // Forward /api/* to the Supabase Edge Function in local development.
          // Production calls the governed Supabase Edge API explicitly from the Cloudflare-hosted SPA.
