...
const fmtMoney = (value: number | null | undefined, currency = 'EUR') =>
  Number(value ?? 0).toLocaleString('es-ES', { style: 'currency', currency, minimumFractionDigits: 2, maximumFractionDigits: 2 })
...
function MetaAdsPanel() {
  ...
  const [state, setState] = useState<{
    ...
    loading: boolean
    error: string | null
  }>({ summary: null, changes: {}, campaigns: [], ads: [], accountIds: [], currency: 'EUR', period: null, loading: true, error: null })
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState({
        ...
        accountIds,
        currency: insights.currency ?? campaigns.currency ?? 'EUR',
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">Datos directos de Meta · {periodLabel}</p>
          <p className="mt-1 text-xs text-muted">Cuentas: {state.accountIds.length > 0 ? state.accountIds.join(' · ') : 'sin cuenta resuelta'}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 xl:grid-cols-6">
        ...
        <MetricCard label="CPC" value={state.loading ? '…' : fmtMoney(summary?.cpc, state.currency)} detail={`CPM ${fmtMoney(summary?.cpm, state.currency)}`} icon={<Target className="h-4 w-4 text-amber-500" />} />
      </div>

      <Card>
        <CardHeader>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <CardTitle className="flex-1">Campañas Meta</CardTitle>
            <div className="relative w-full sm:w-64">
              <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted" />
              <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar campaña…" className="w-full rounded-lg border border-border bg-card py-2 pl-9 pr-3 text-sm text-foreground placeholder:text-muted" />
            </div>
          </div>
        </CardHeader>
      ...
      <Card>
        <CardHeader><CardTitle>Anuncios Meta cargados</CardTitle></CardHeader>
        <CardContent>
          <p className="text-3xl font-bold">{state.loading ? '…' : state.ads.length.toLocaleString('es-ES')}</p>
          <p className="mt-1 text-xs text-muted">Inventario devuelto por `/api/meta/ads` para el período. Creatives, ad sets y media-level insights se incorporarán en Meta 360.</p>
        </CardContent>
      </Card>
    </div>
  )
...
function GoogleAdsPanel() {
  ...
  const [state, setState] = useState<{
    ...
    loading: boolean
    error: string | null
  }>({ connection: null, summary: null, changes: {}, campaigns: [], period: null, loading: true, error: null })
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState({ connection, summary: insights.summary ?? null, changes: insights.changes ?? {}, campaigns: campaigns.campaigns ?? [], period: insights.period ?? null, loading: false, error: null })
    } catch (error: unknown) {
      if (requestSequence !== requestSequenceRef.current) return
  ...
  return (
    <div className="space-y-6">
      ...
      {state.error && <div className="rounded-lg border border-[#D9534F]/30 bg-[#D9534F]/8 px-4 py-3 text-sm text-[#D9534F]">{state.error}</div>}
      {state.connection && <div className="rounded-lg border border-border bg-card px-4 py-3"><p className="text-sm font-semibold">{state.connection.connected ? 'Conexión Google Ads operativa' : 'Conexión Google Ads incompleta'}</p><p className="mt-1 text-xs text-muted">Customer ID: {state.connection.customerId ?? 'no configurado'} · credencial {state.connection.credentialPresent ? 'configurada' : 'ausente'} · último uso {state.connection.credentialLastUsed ? new Date(state.connection.credentialLastUsed).toLocaleString('es-ES') : 'sin uso registrado'} · sync {state.connection.lastSync ? new Date(state.connection.lastSync).toLocaleString('es-ES') : 'sin sincronización'}</p></div>}
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 xl:grid-cols-6"><MetricCard label="Gasto" value={state.loading ? '…' : fmtMoney(state.summary?.spend)} icon={<DollarSign className="h-4 w-4 text-emerald-500" />} delta={state.changes.spend} /><MetricCard label="Impresiones" value={state.loading ? '…' : fmtNumber(state.summary?.impressions)} icon={<Eye className="h-4 w-4 text-sky-500" />} delta={state.changes.impressions} /><MetricCard label="Clics" value={state.loading ? '…' : fmtNumber(state.summary?.clicks)} detail={`CTR ${fmtNumber(state.summary?.ctr, 2)}%`} icon={<MousePointerClick className="h-4 w-4 text-violet-500" />} delta={state.changes.clicks} /><MetricCard label="Conversiones Google" value={state.loading ? '…' : fmtNumber(state.summary?.conversions, 2)} detail="Proveedor · no equivale a cliente nuevo" icon={<TrendingUp className="h-4 w-4 text-lime-500" />} delta={state.changes.conversions} /><MetricCard label="CPC" value={state.loading ? '…' : fmtMoney(state.summary?.cpc)} detail={`CPM ${fmtMoney(state.summary?.cpm)}`} icon={<Target className="h-4 w-4 text-amber-500" />} /><MetricCard label="Campañas cargadas" value={state.loading ? '…' : String(state.campaigns.length)} detail="Máximo 50 por consulta actual" icon={<BarChart3 className="h-4 w-4 text-rose-500" />} /></div>
      <Card><CardHeader><div className="flex flex-col gap-3 sm:flex-row sm:items-center"><CardTitle className="flex-1">Campañas Google Ads</CardTitle><div className="relative w-full sm:w-64"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted" /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar campaña…" className="w-full rounded-lg border border-border bg-card py-2 pl-9 pr-3 text-sm" /></div></div></CardHeader><CardContent className="p-0">{state.loading && <p className="p-4 text-sm text-muted">Cargando campañas Google Ads…</p>}{!state.loading && !state.error && filteredCampaigns.length === 0 && <p className="p-4 text-sm text-muted">No hay campañas para el período.</p>}{!state.loading && filteredCampaigns.length > 0 && <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-xs uppercase text-muted"><th className="px-4 py-3 text-left">Campaña</th><th>Estado</th><th>Tipo</th><th className="text-right">Gasto</th><th className="text-right">Impresiones</th><th className="text-right">Clics</th><th className="text-right">CTR</th><th className="px-4 text-right">Conversiones</th></tr></thead><tbody>{filteredCampaigns.map((campaign) => <tr key={campaign.id} className="border-b"><td className="px-4 py-3 font-medium">{campaign.name}</td><td className="text-center">{campaign.status}</td><td className="text-center">{campaign.type}</td><td className="text-right">{fmtMoney(campaign.insights.spend)}</td><td className="text-right">{fmtNumber(campaign.insights.impressions)}</td><td className="text-right">{fmtNumber(campaign.insights.clicks)}</td><td className="text-right">{fmtNumber(campaign.insights.ctr, 2)}%</td><td className="px-4 text-right">{fmtNumber(campaign.insights.conversions, 2)}</td></tr>)}</tbody></table></div>}</CardContent></Card>
    </div>
  )
}
