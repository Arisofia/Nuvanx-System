...
export default function Traceability() {
  ...
  const renderPatientInfo = (r: any) => {
    ...
    if (r.doctoralia_template_name && !r.doc_patient_id) {
      const label = r.phone_normalized ? 'Cruzado por telÃ©fono' : 'Cruzado por nombre'
  ...
  const campaignColumns: ColDef[] = [
    ...
    { key: 'verified_revenue_crm', label: 'Revenue Doctoralia', align: 'right', sortable: true, format: (v) => `â‚¬${Number(v).toLocaleString('es-ES')}` },
  ]
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-white/80 backdrop-blur-xl p-1.5 rounded-xl border border-[#E5D5C5]/60 shadow-sm">
            ...
            <span className="text-[#B08B5A]/40 text-xs">â†’</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="hover:shadow-xl transition-all duration-500 border-none shadow-sm bg-white"><CardContent className="pt-8"><p className="text-[10px] font-bold text-[#5C5550] uppercase tracking-[0.2em]">Caja liquidada</p><p className="text-4xl font-serif font-bold mt-4 tracking-tight text-primary">{totalRevenue > 0 ? `â‚¬${totalRevenue.toLocaleString('es-ES', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}` : 'â€”'}</p></CardContent></Card>
      </div>

      <Tabs defaultValue="leads" className="w-full">
        <TabsList className="flex-wrap h-auto">
          <TabsTrigger value="leads" className="gap-2"><GitMerge className="h-4 w-4" />Cruces Doctoralia</TabsTrigger>
          <TabsTrigger value="campaigns" className="gap-2"><TrendingUp className="h-4 w-4" />Rendimiento campaÃ±as</TabsTrigger>
        ...
        <TabsContent value="leads" className="mt-4">
          <Card className="border-none shadow-md bg-white overflow-hidden">
            <CardHeader className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0 pb-6 border-b border-border/10">
              <div><CardTitle className="flex items-center gap-2 font-serif text-2xl text-[#2C2825]"><GitMerge className="h-5 w-5 text-primary" />Listado de trazabilidad</CardTitle><p className="text-xs text-[#5C5550] font-medium mt-1">Cruce directo de leads y registros de clÃ­nica</p></div>
              <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                ...
                <div className="relative flex-1 sm:flex-none"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#8E8680] pointer-events-none" /><input type="text" placeholder="Buscar lead o pacienteâ€¦" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 pr-4 py-2.5 text-sm bg-[#FAF7F2]/40 border border-border/30 rounded-xl w-full sm:w-64 text-[#2C2825] placeholder:text-[#8E8680] focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary transition-all" /></div>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              {loading && <p className="text-sm text-[#8E8680] py-12 text-center animate-pulse italic">Cargando trazabilidadâ€¦</p>}
              {!loading && error && <p className="text-sm text-[#D9534F] py-12 text-center font-medium bg-red-50/50 rounded-2xl">{error}</p>}
              {!loading && !error && filtered.length === 0 && <div className="py-16 text-center space-y-3"><div className="bg-[#FAF7F2] w-16 h-16 rounded-full flex items-center justify-center mx-auto border border-border/20"><GitMerge className="h-8 w-8 text-[#C9B9A8]" /></div><p className="text-sm font-medium text-[#5C5550]">{rows.length === 0 ? 'AÃºn no hay leads registrados.' : 'NingÃºn resultado para la bÃºsqueda actual.'}</p></div>}
              {!loading && !error && filtered.length > 0 && <div className="overflow-x-auto"><table className="w-full text-xs border-collapse"><thead><tr className="border-b border-border/10 text-[#5C5550] uppercase tracking-[0.15em] text-[10px] font-bold"><th className="text-left py-4 pr-4 pl-2 font-bold">Lead</th><th className="text-left py-4 pr-4 font-bold">Fuente / CampaÃ±a</th><th className="text-left py-4 pr-4 font-bold">Fecha cita</th><th className="text-left py-4 pr-4 font-bold">Estado cruce</th><th className="text-left py-4 pr-4 font-bold">Paciente Doctoralia</th><th className="text-right py-4 pr-4 font-bold">Ingreso â‚¬</th><th className="text-right py-4 pr-2 font-bold">LiquidaciÃ³n</th></tr></thead><tbody className="divide-y divide-border/5">{filtered.map((r, idx) => { const matched = Boolean(r.patient_id || r.doc_patient_id || r.doctoralia_template_name); const hasRevenue = r.doctoralia_net && r.doctoralia_net > 0; let matchLabel = 'Cruzado'; if (r.match_class) { matchLabel = MATCH_LABELS[r.match_class] ?? r.match_class } else if (r.doctoralia_template_name && r.doc_patient_id == null) { matchLabel = r.phone_normalized ? 'Por telÃ©fono' : 'Por nombre' } return (<tr key={r.lead_id} className={`group hover:bg-[#FAF7F2]/60 transition-colors ${idx % 2 === 0 ? 'bg-transparent' : 'bg-[#FAF7F2]/20'}`}><td className="py-5 pr-4 pl-2"><p className="font-serif font-bold text-[#2C2825] text-sm">{r.lead_name ?? 'â€”'}</p>{r.stage && <span className="text-[9px] font-bold text-primary uppercase bg-primary/5 px-2 py-0.5 rounded mt-1.5 inline-block border border-primary/10 tracking-widest">{r.stage}</span>}</td><td className="py-5 pr-4"><p className="text-[#2C2825] font-semibold">{r.source ?? 'â€”'}</p>{r.campaign_name && <p className="text-[#5C5550] text-[10px] font-medium truncate max-w-[180px] mt-1.5" title={r.campaign_name}>{r.campaign_name}</p>}</td><td className="py-5 pr-4 text-[#5C5550] font-bold whitespace-nowrap">{r.appointment_date ? <div className="flex flex-col gap-1"><span>{new Date(r.appointment_date).toLocaleDateString('es-ES')}</span>{r.lead_created_at && <span className="text-[9px] text-[#8E8680] font-bold uppercase tracking-wider">Lead {new Date(r.lead_created_at).toLocaleDateString('es-ES')}</span>}</div> : <span className="text-[#C9B9A8] font-medium opacity-60">Sin cita</span>}</td><td className="py-5 pr-4">{matched ? <div className="flex flex-col gap-1.5"><div className="flex items-center gap-1.5 text-green-600 font-bold tracking-tight"><CheckCircle2 className="h-3.5 w-3.5" /><span>{matchLabel}</span></div>{r.match_confidence != null && <div className="w-24 bg-border/20 h-1 rounded-full overflow-hidden mt-0.5"><div className="bg-green-600 h-full rounded-full transition-all" style={{ width: `${Math.round(r.match_confidence * 100)}%` }} /></div>}</div> : <div className="flex items-center gap-1.5 text-[#C9B9A8] font-bold"><XCircle className="h-3.5 w-3.5 opacity-40" /><span>Sin cruce</span></div>}</td><td className="py-5 pr-4"><div className="flex flex-col gap-1.5"><div className="font-serif font-bold text-[#2C2825]">{renderPatientInfo(r)}</div>{r.doctoralia_template_name && <p className="text-[#5C5550] text-[10px] font-medium truncate max-w-[140px] italic border-l-2 border-primary/20 pl-2 leading-tight">{r.doctoralia_template_name}</p>}</div></td><td className="py-5 pr-4 text-right">{hasRevenue ? <span className="text-[#2C2825] font-serif font-bold text-base tracking-tight">â‚¬{r.doctoralia_net.toLocaleString('es-ES', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span> : <span className="text-[#C9B9A8] font-medium opacity-40">â€”</span>}</td><td className="py-5 text-right pr-2">{r.days_to_settlement == null ? <span className="text-[#C9B9A8] font-medium opacity-40">â€”</span> : <div className="flex flex-col items-end gap-1.5"><span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${r.days_to_settlement <= 30 ? 'bg-green-600/5 text-green-600 border-green-600/20' : 'bg-[#FAF7F2] text-[#8E8680] border-border/40'}`}>{r.days_to_settlement}d</span><span className="text-[9px] text-[#8E8680] font-bold uppercase tracking-wider">tras lead</span></div>}</td></tr>) })}</tbody></table></div>}
            </CardContent>
          </Card>
        </TabsContent>

        <TabsContent value="campaigns" className="mt-4"><Card><CardHeader><CardTitle>Rendimiento por campaÃ±a</CardTitle></CardHeader><CardContent><SortableTable columns={campaignColumns} rows={campaigns} loading={loading} emptyMessage="No hay datos de rendimiento de campaÃ±as todavÃ­a." exportFilename="rendimiento-campanas" /></CardContent></Card></TabsContent>
