  ...
  if (object === 'whatsapp_business_account') {
    ...
    for (const e of (entry || [])) {
      for (const change of (e.changes || [])) {
        ...
        // ── WhatsApp Flows (nfm_reply) — form submissions ─────────────────
        for (const msg of (change.value?.messages || [])) {
          ...
          // defined when the WhatsApp Flow was created (clinic-specific).
          ...
          if (webhookUserId && isAvailable()) {
            ...
            onLeadCreated({
              userId: webhookUserId,
              lead: { id: `${phone}_${msg.id}`, name, phone, email: email || '' },
              source: 'whatsapp_flow',
            }).catch((autoErr) => logger.warn('WA Flow webhook: playbook automation error', { error: autoErr.message }));
          }
        }
      }
    }
