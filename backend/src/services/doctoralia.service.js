...
async function ingestRows({ rows, clinicId }) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        if (upsertRes.rows[0]?.is_insert) inserted += 1;
        else updated += 1;

        await client.query('RELEASE SAVEPOINT sp');
        if (patientId) patientIdsToReconcile.push(patientId);
      } catch (rowErr) {
  ...
  for (const patientId of patientIdsToReconcile) {
    pool.query('SELECT reconcile_patient_leads($1)', [patientId])
      .catch((e) => logger.warn('reconcile_patient_leads failed', { patientId, error: e.message }));
  }

  return { inserted, updated, patientsUpserted, rowErrors };
}

module.exports = { CANCELLED_ESTADOS, parseName, parseAmount, parseDate, ingestRows };