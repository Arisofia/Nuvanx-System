...
function renderDashboard(leads, stats) {
  const rows = leads.map(l => {
    ...
    return `<tr>
      <td>${l.name}</td>
      <td>${l.email}</td>
      <td><span style="color:${tierColor};font-weight:700">${l.tier}</span></td>
      <td style="font-weight:700">${l.finalScore}</td>
      <td>${l.quality_label}</td>
      <td>${l.recommended_action}</td>
      <td>${l.meta_capi_priority} ${metaBadge}</td>
    </tr>`;
  }).join('');
...
export default {
  async fetch(request, env) {
    ...
    // GET /score → Analiza leads y muestra dashboard
    if (url.pathname === '/score') {
      try {
        if (!env.HUBSPOT_TOKEN) {
          return new Response('Error: HUBSPOT_TOKEN no configurado en Variables de entorno de Cloudflare.', { status: 500 });
        }
        ...
        for (const c of sample) {
          ...
          // Guardar score en HubSpot (fire-and-forget)
          env.HUBSPOT_TOKEN && updateHubSpotScore(
            env, c.id, finalScore, local.tier, ai.recommended_action
          ).catch(() => {});
        }
        ...
        return new Response(renderDashboard(results, stats), {
          headers: { 'Content-Type': 'text/html;charset=utf-8' }
        });
    ...
    // GET /api/leads → JSON para integración externa
    if (url.pathname === '/api/leads') {
      try {
        const contacts = await fetchHubSpotLeads(env);
        const results  = contacts.slice(0, 50).map(c => {
          const props = c.properties || {};
          const local = scoreLeadLocally(props);
          return { id: c.id, ...props, ...local };
        });
